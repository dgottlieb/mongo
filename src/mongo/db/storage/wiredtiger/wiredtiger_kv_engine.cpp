...
public:
    ...
    void setStableTimestamp(Timestamp stableTimestamp) {
        ...
        // log() << "Stable: " << stableTimestamp
        //   << " Initial: " << Timestamp(_initialDataTimestamp.load());
...
WiredTigerKVEngine::WiredTigerKVEngine(const std::string& canonicalName,
...

    {
        ...
        std::uint64_t tmp;
        fassert(50751, parseNumberFromStringWithBase(buf, 16, &tmp));
        _recoveryTimestamp = Timestamp(tmp);
        log() << "WiredTiger recoveryTimestamp. Ts: " << _recoveryTimestamp;
    }
...
void WiredTigerKVEngine::setStableTimestamp(Timestamp stableTimestamp) {
    // log() << "ReplSetStable. " << stableTimestamp;
    if (stableTimestamp.isNull()) {
        return;
    }

    if (_checkpointThread && stableTimestamp.asULL() <= _checkpointThread->getStableTimestamp()) {
        // log() << "CkptStable: " << Timestamp(_checkpointThread->getStableTimestamp());
        return;
    }
